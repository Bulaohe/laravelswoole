...
class SwooleHttpWrapper extends Swoole implements ServerInterface
{
    ...
    public function onWorkerStart($serv, $worker_id)
    {
        ...
        $this->accept_gzip = config('swoole.base_config.gzip');
    }
    ...
    public function onRequest($request, $response)
    {
        ...
        if (config('swoole.base_config.deal_with_public')) {
    ...
    protected function handleResponse($request, $response, $illuminateResponse)
    {
        ...
        // headers
        $response->header('Server', config('swoole.base_config.server'));
        ...
        if ($illuminateResponse instanceof BinaryFileResponse) {
            ...
            if ($accept_gzip && isset($response->header['Content-Type'])) {
                ...
                if (strlen($content) > config('swoole.base_config.gzip_min_length') && is_mime_gzip($mime)) {
                    $response->gzip(config('swoole.base_config.gzip'));
                }
            }
        }
